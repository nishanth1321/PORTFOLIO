...
export const TypewriterEffect = ({
  ...
  const wordsArray = words.map((word) => {
    return {
      ...word,
      text: word.text.replace(/\t/g, "\u00A0").split(""),
    };
  });
  ...
  return (
    (<div
      className={cn(
        "text-base sm:text-xl md:text-2xl lg:text-3xl font-semibold text-center",
        className
      )}>
      {renderWords()}
      <motion.span
        ...
        className={cn(
          "inline-block rounded-sm w-[4px] h-4 md:h-6 lg:h-9 bg-blue-500",
          cursorClassName
        )}></motion.span>
    </div>)
  );
